...
public class JWTUtil {
    public static String sercetKey = "InMySchoolOnline";//token加密时使用的密钥
    //一旦得到该密钥也就可以伪造token了
    public final static long keeptime = 1800000;//代表token的有效时间,
    ...
    public static String generToken(String id, String issuer, String subject) {
        ...
        Date now = new Date(nowMillis); //获取系统时间以便设置token有效时间
        byte[] apiKeySecretBytes = DatatypeConverter.parseBase64Binary(sercetKey);  //将密钥转码为base64形式,再转为字节码
        Key signingKey = new SecretKeySpec(apiKeySecretBytes, signatureAlgorithm.getJcaName());  //对其使用Hash256进行加密
        JwtBuilder builder = Jwts.builder().setId(id).setIssuedAt(now); //JWT生成类,此时设置iat,以及根据传入的id设置token
        if (subject != null) {
